...
interface CategoryBaseInterface extends BaseModelInterface {
	name: string
}
...
> {
	...
	public async data(dto: boolean = true): Promise<CategoryInterface> {
		...
		const [products, subcategories, parentCategory, thumbnail] =
			await Promise.all([
				fetchMultiData<ProductInterface, Product>(
					() => this.getProducts(),
					dto
				),
